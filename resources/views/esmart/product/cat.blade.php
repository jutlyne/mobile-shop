    ...
    <div class="main-content fl-right">
        <div class="section" id="list-product-wp">
            <div class="section-head clearfix">
                <h3 class="section-title fl-left">{{ $infoCat->cat_product_name }}</h3>
                <div class="filter-wp fl-right">
                    <p class="desc">Hiển thị {{ $productCat->count() }} trên {{ $countProduct }} sản phẩm</p>
                    <div class="form-filter">
                        <form method="GET">
                            <select onchange="location=this.value">
                                <option
                                    value="{{ route('esmart.product.cat', [$infoCat->cat_product_slug, $infoCat->cat_product_id]) }}">
                                    Sắp xếp theo giá</option>
                                <option {{ $sort == 'giam-nhieu' ? 'selected' : '' }}
                                    value="{{ route('esmart.product.cat', [$infoCat->cat_product_slug, $infoCat->cat_product_id]) }}?sort=giam-nhieu">
                                    Giảm giá nhiều
                                </option>
                                <option {{ $sort == 'giam-dan' ? 'selected' : '' }}
                                    value="{{ route('esmart.product.cat', [$infoCat->cat_product_slug, $infoCat->cat_product_id]) }}?sort=giam-dan">
                                    Giá cao xuống thấp
                                </option>
                                <option {{ $sort == 'tang-dan' ? 'selected' : '' }}
                                    value="{{ route('esmart.product.cat', [$infoCat->cat_product_slug, $infoCat->cat_product_id]) }}?sort=tang-dan">
        ...
        <div class="section" id="paging-wp">
            <div class="section-detail">
                @if ($sort != '')
                    {{ $productCat->appends(['sort' => $sort])->links('vendor.pagination.pagination-public') }}
                @else
                    {{ $productCat->links('vendor.pagination.pagination-public') }}
                @endif
            </div>
        </div>
    </div>

...
@section('main-content')
    <div class="main-content fl-right">
        <div class="section" id="list-product-wp">
            <div class="section-head clearfix">
                <h3 class="section-title fl-left">
                    Kết quả tìm kiếm từ khóa <span class="text-danger">"{{ $keyword }}"<span>
                </h3>
                    ...
                    <div class="form-filter">
                        <form method="GET">
                            <select onchange="location=this.value">
                                <option value="{{ route('esmart.search.search') }}?keyword={{ $keyword }}">Sắp xếp theo giá
                                </option>
                                <option {{ $sort == 'giam-nhieu' ? 'selected' : '' }}
                                    value="{{ route('esmart.search.search') }}?keyword={{ $keyword }}&sort=giam-nhieu">Giảm
                                    giá nhiều
                                </option>
                                <option {{ $sort == 'giam-dan' ? 'selected' : '' }}
                                    value="{{ route('esmart.search.search') }}?keyword={{ $keyword }}&sort=giam-dan">Giá cao
                                    xuống thấp
                                </option>
                                <option {{ $sort == 'tang-dan' ? 'selected' : '' }}
                                    value="{{ route('esmart.search.search') }}?keyword={{ $keyword }}&sort=tang-dan">Giá
                                    thấp lên cao</option>
                            </select>
                        </form>
                    </div>
        ...
        <div class="section" id="paging-wp">
            <div class="section-detail">
                @if ($sort != '')
                    {{ $productAll->appends(['keyword' => $keyword, 'sort' => $sort])->links('vendor.pagination.pagination-public') }}
                @else
                    {{ $productAll->appends(['keyword' => $keyword])->links('vendor.pagination.pagination-public') }}
                @endif
            </div>
        </div>
    </div>
